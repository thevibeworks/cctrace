...
- Redrew the stage's state diagram as the LOOP ROW in the replay bar. Rev 1's six nodes and nine counted edges modelled a Markov chain, not Claude Code's loop: reply is an outcome rather than a state, waiting read 0 on every trace inspected, human never lit, the counts were redundant (tools 160 = model>tools 160 = tools>model 159) and the headline restated the conversation header's time chip verbatim. The row draws the actual machine - human -> model -> one hand-off slot that reads tools, agents or waiting -> model ... -> human - once, horizontally, at fixed pixel geometry in the frame under the strip: one chip lit for the state at the cursor, the edge it came in by lit where the wire shows one (a prompt, the calls, the results, the answer; a failed request is the model chip in red with no edge), and beside it what is running, how long it has held where that is a wire fact, and the absolute clock it began at. No counts on it: the tally stays in "so far". Click it to jump to the step that opened the state. At the live edge the model chip beats while a request is in flight - the page's one heartbeat while replaying
