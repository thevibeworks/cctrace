...
- **上下文洞察。** Context 视图逐请求展示模型窗口由什么组成，版式是一张
  账本：左侧常驻页边栏给出这一步的结账 -- prompt token 数、对照模型上下文
  窗口的六色组成条、chars/4 估算比实际计费低（或高）多少、以及六个分类各占
  一行，点任意一行就把图放大到该分类；右侧画布则是轨迹图（✂ 标出
  compact/rewind，看柱子掉下来）、选中那一步的 **context graph** -- 一张
  icicle（火焰图）：宽度是 token，行是层级，把该请求拆成
  分类 -> 分组 -> 条目 -- 工具结果按产出它的工具分组、schema 按 MCP server
  分组、注入按来源分组 -- 以及每次注入/压缩/换模型的事件列表。于是
  `Bash x189，占窗口 38%` 就是那一行里最宽的一块，
  不是要翻出来的数字；点一下放大，点叶子节点看原始字节。一条 trace 里有多个
...
- **轨迹进入时间线。** 会话 rail 上每一步都带一条轨道：窗口占用了多少，
  ...
  下一步冷启动，然后再次回暖。
- **轨迹本身，一条流。** Trajectory 视图把一个线程渲染成一条按时间锚定的
  线性记录流：system prompt、你的每一轮、harness 注入的上下文（就地、
  在它进入的那一刻）、模型的思考、每次工具调用与其结果合为一行、回复。
  MAP / READ / FULL 三档只做过滤，从不摘要：下到骨架，上到全部；按类型
  筛选可以只看注入（这就是上下文轨迹），顶部一条时间带说明墙钟花在了
  哪里：模型、工具、等待、轮次之间。
