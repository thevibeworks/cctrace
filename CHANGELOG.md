...
- Replay TAILS a live session now. Parking at the live edge used to be a lie: a landed pair redrew the strip but never moved the cursor, so a reader who entered replay on a running session fell behind in silence while the beat, the conversation and the state froze at entry time. A pair now measures where the reader was BEFORE it lands - at the edge the cursor follows it, the now line and the beat move with it, and the conversation sticks to its bottom only if the reader was already there. Behind the edge nothing moves and the live chip flips to a snap-back button. Older pairs merged in from a prior run never move the cursor; playback reaching the end of a live tape parks at the edge instead of just stopping
- Replaced the stage's six-node state diagram with the NOW line. The diagram modelled the wrong machine: Claude Code's loop is prompt -> [model -> tools]* -> model -> answer, so reply is an outcome rather than a state the agent occupies, waiting read 0 on every trace inspected, and human never lit. Its counts were redundant (tools 160 = model>tools 160 = tools>model 159) and its headline restated the conversation header's time chip verbatim. In its place one row: a dot in the lane color, the observed state (model / tools / agents / waiting / human / failed / idle), what is running (thinking, the tool names with repeats folded, the running children by name, the status that failed), how long it has held where the extent is a wire fact, and the absolute clock it began at. Click it to jump to the step that opened the state. The dot beats only while a request is actually in flight, and the loop's shape stays where it was already drawn - the strip and the rail
