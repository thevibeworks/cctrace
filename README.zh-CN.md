...
- **Sessions 视图** -- 把重建的对话放在一条"轨道"上：session 收纳各自的
  线程（主对话、子代理以分支形式挂在派发它们的那个回合下、工具类噪声折叠），
  `/model` 切换标记一个 epoch，compact 压缩标记一个断点节点（上下文从多少
  回合/token 收缩到多少一目了然），被覆盖的交互（rewind、编辑、注入的
  recap）灰显在它原本的位置。每个回合都链接回产生它的请求；token、耗时、
  费用悬停即看。
...
不删除任何 pair、绝不动响应体，重建出的 Sessions 视图完全一致 -- 失去的
