...
- **上下文洞察。** Context 视图逐请求展示模型窗口由什么组成：六色组成条
  对照模型上下文窗口、历史图上 ✂ 标出 compact/rewind（看柱子掉下来）、
  每次注入/压缩/换模型都是一条事件，还有 **context graph** -- 一张
  icicle（火焰图）：宽度是 token，行是层级，把任意请求拆成
  分类 -> 分组 -> 条目 -- 工具结果按产出它的工具分组、schema 按 MCP server
  分组、注入按来源分组。于是 `Bash x189，占窗口 38%` 就是那一行里最宽的一块，
  不是要翻出来的数字；点一下放大，点叶子节点看原始字节。一条 trace 里有多个 session 时，顶部会出现线程条，同一标尺
  对比各自的峰值。全部锚定在服务端上报的 token 数上 -- 是 wire 事实，
  不是猜测。
